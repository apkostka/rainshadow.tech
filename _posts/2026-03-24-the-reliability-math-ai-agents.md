...
description: "Compounding failure rates in multi-step AI agents explain why 32% of practitioners cite quality as their top production barrier—and what you can do about it."
---
...
If each step in an agentic workflow succeeds 85% of the time — a solid baseline for a well-engineered, well-tested agent — a 10-step workflow will complete successfully roughly 20% of the time. That's not an approximation. It's 0.85^10 = 0.197. A 5-step workflow at the same per-step reliability succeeds about 44% of the time. A 15-step workflow falls below 10%.

This is the arithmetic behind why 32% of practitioners in recent industry surveys cite quality as their single biggest barrier to getting agents into production. The reliability doesn't break because the model is bad or the engineering is careless. It compounds away, step by step, in ways that are entirely predictable — once you know to look for them.

## Why 85% Per-Step Reliability Is a Realistic Baseline

Before examining what these numbers mean for architecture, it's worth understanding why 85% per-step success is a reasonable middle-of-the-road assumption rather than a pessimistic edge case.
...
An 85% per-step baseline assumes you've already done meaningful engineering work: thoughtful system prompts, input validation, retry logic for transient errors, and structured output formats. Teams earlier in their development process often see lower per-step success rates than this, which makes the compounding even more severe.
...
The most direct lever you have on end-to-end reliability is reducing the number of steps required to complete a task. A workflow that requires 5 steps at 85% per-step reliability succeeds 44% of the time. Reduce it to 3 steps and success probability climbs to 61%. Reduce it to 2 steps and you're above 72%.
...
The verification logic doesn't need to be elaborate. A JSON schema check and a handful of assertions on critical fields will catch the most common failure modes. The important architectural point is that verification happens before the workflow proceeds, so failures surface at the step where they occurred rather than propagating forward and becoming difficult to diagnose.
...
The reliability math makes a strong case for human review or confirmation gates at specific points in agentic workflows — not as a workaround for a system you don't trust, but as deliberate architecture for actions where mistakes are costly or irreversible.
...
In a multi-step workflow, the question isn't whether failures will occur — they will — but whether your system handles partial failures gracefully or leaves the world in a broken intermediate state.
...
**On task scope:**
- How many distinct steps does this workflow require from start to finish?
- Can any steps be combined or eliminated without reducing the workflow's utility?
- Does each step have a clear, testable success condition?

**On verification:**
- Is each step's output validated before the workflow proceeds to the next step?
- Are failures surfaced with enough diagnostic context to identify the root cause?
- Do retry mechanisms apply appropriate backoff and avoid repeating known-bad inputs?

**On high-stakes actions:**
- Which steps in this workflow produce irreversible effects?
- Which steps have disproportionate downstream impact when they produce incorrect output?
- Are those steps protected by confirmation gates or human review checkpoints?

**On partial failure:**
- If this workflow fails partway through, what state will external systems be in?
- Can each step be safely retried after a partial completion?
- Is rollback logic implemented for steps that require it?

**On observability:**
- Is per-step success rate instrumented and logged?
- Are end-to-end success rates tracked over time so reliability degradation becomes visible before it becomes critical?
- Are error patterns aggregated so common failure modes can be addressed systematically?

## Starting Where You Are

If you have agentic workflows running in production today, the most immediately useful step is to run the reliability math on your current step counts and per-step success rates. If you don't have per-step instrumentation yet, adding it is the right first move — without per-step data, you can diagnose end-to-end failures but can't identify where in the workflow reliability is being lost.

For teams earlier in the design process, the single most impactful structural choice is task scope. Starting with short, tightly defined workflows, validating their reliability in production conditions, and extending scope incrementally as you build confidence in the system's performance is a more durable path than building long autonomous workflows and iterating backward from production failures.

The goal isn't a system that never fails. Given the probabilistic nature of the underlying models, that target isn't achievable at any practical level of engineering investment. The achievable goal is a system where failures are caught at the step where they occur, isolated from downstream effects, and handled in ways that leave the world in a recoverable state.
